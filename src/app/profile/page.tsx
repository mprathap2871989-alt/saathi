...
import { getMyProfile, getOrCreateUser } from "@/actions/user";
...
export default async function ProfilePage() {
      ...
      <>
        ...
        <main className="max-w-2xl mx-auto px-4 py-12">
          <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-6 sm:p-8 text-center">
            ...
            <p className="text-sm text-gray-400 leading-relaxed max-w-md mx-auto mt-4">
              You cannot create stories, respond to others, vote, report
              content, or change your profile while your account is suspended.
            </p>

            <div className="mt-7">
              <Link
                href="/community"
                className="inline-flex items-center justify-center px-5 py-2.5 bg-emerald-700 text-white text-sm font-semibold rounded-full hover:bg-emerald-800 transition-colors"
              >
                Return to Community
              </Link>
            </div>
  ...
  const activePosts = profile.posts.filter(
    (p: (typeof profile.posts)[number]) => !p.isRemoved
  );
  ...
  return (
    <>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        <h1 className="font-serif text-2xl font-bold text-gray-900 mb-5">
          My Profile
        </h1>
          ...
          <div className="flex items-center gap-4 mb-5">
            <div
              className={`${avatarColor(
                profile.username
              )} w-14 h-14 rounded-full flex items-center justify-center text-white text-2xl font-bold flex-shrink-0`}
            >
            ...
            {[
              {
                ...
                val: activePosts.length,
              },
              {
                icon: <ThumbsUp size={14} />,
                label: "Helpful Given",
                val: profile._count.helpfulVotes,
              },
              {
                icon: <MessageCircle size={14} />,
                label: "Responses Made",
                val: profile._count.comments,
              },
            ].map(({ icon, label, val }) => (
              <div
                key={label}
