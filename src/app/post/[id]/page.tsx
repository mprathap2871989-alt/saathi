...
// Performance:
// - generateMetadata() uses a lightweight Post query.
// - PostPage reuses currentUsername returned by getPost(),
//   avoiding a duplicate Prisma user lookup.
...
export async function generateMetadata({
  ...
  // Metadata only needs title and story.
  // Avoid the full getPost() workload here.
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        ...
        <section className="mt-8" aria-label="Responses">
          ...
          <div className="mt-4 space-y-3">
            ...
            {post.comments.map(
              (comment: (typeof post.comments)[number]) => (
                <CommentItem
                  key={comment.id}
                  id={comment.id}
                  postId={post.id}
